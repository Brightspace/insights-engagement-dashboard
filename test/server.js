...
const handlers = [
	...
	rest.get('/d2l/api/ap/unstable/insights/data/engagement', async(req, res, ctx) => {
		await delay(100);

		const searchParams = req.url.searchParams;
		const defaultView = searchParams.get('defaultView') === 'true';
		return res(ctx.json({
			records: [
			...
			selectedSemestersIds: parseIntCsv(searchParams.get('selectedSemestersCsv')),
			selectedRolesIds: parseIntCsv(searchParams.get('selectedRolesCsv')),
			selectedOrgUnitIds: parseIntCsv(searchParams.get('selectedOrgUnitIdsCsv')),
	...
	rest.get('/d2l/api/ap/unstable/insights/data/roles', async(req, res, ctx) => {
		await delay(100);
	...
	rest.get('/d2l/api/ap/unstable/insights/data/semesters', async(req, res, ctx) => {
		await delay(100);
...
function parseIntCsv(csv) {
	if (!csv) {
		return [];
	}
	return csv.split(',').filter(n => Number.isInteger(n)).map(Number);
}
